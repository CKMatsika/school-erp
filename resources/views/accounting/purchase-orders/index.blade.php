<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('accounting.purchase-orders.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="GET" action="{{ route('accounting.purchase-orders.index') }}" class="flex flex-wrap items-end gap-4">
                        <div class="w-full sm:w-auto">
                            <x-input-label for="search" :value="__('Search')" />
                            <x-text-input id="search" class="block mt-1 w-full" type="text" name="search" :value="request('search')" placeholder="PO number, supplier..." />
                        </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($purchaseOrders as $po)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $po->date_expected ? $po->date_expected->format('M d, Y') : 'Not specified' }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                            <a href="{{ route('accounting.purchase-orders.show', $po) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                            
                                            @if($po->status == 'draft' || $po->status == 'pending')
                                                <a href="{{ route('accounting.purchase-orders.edit', $po) }}" class="text-green-600 hover:text-green-900 mr-3">Edit</a>
                                            @endif

                                            @if(($po->status == 'approved' || $po->status == 'issued' || $po->status == 'partial') && !$po->is_fully_received)
                                                <a href="{{ route('accounting.purchase-orders.receive', $po) }}" class="text-indigo-600 hover:text-indigo-900">Receive</a>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
