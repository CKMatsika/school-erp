        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('accounting.contacts.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="mt-2 space-x-4">
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="contact_type" value="customer" class="form-radio text-indigo-600" {{ old('contact_type') == 'customer' ? 'checked' : '' }} required>
                                        <span class="ml-2">{{ __('Customer') }}</span>
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="contact_type" value="vendor" class="form-radio text-indigo-600" {{ old('contact_type') == 'vendor' ? 'checked' : '' }}>
                                        <span class="ml-2">{{ __('Vendor') }}</span>
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="contact_type" value="student" class="form-radio text-indigo-600" {{ old('contact_type') == 'student' ? 'checked' : '' }}>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="address" name="address" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('address') }}</textarea>
                            ...
                            <div id="student_select_div" class="md:col-span-2" style="display: {{ old('contact_type') == 'student' ? 'block' : 'none' }};">
                                {{-- Modified Label: Removed asterisk since it's not required --}}
                                ...
                                <select id="student_id" name="student_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    {{-- Use @isset or check count to avoid errors if $students is empty/null --}}
                                    @isset($students)
                                        @foreach ($students as $student)
                                            {{-- Adjust displayed student info as needed --}}
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function toggleStudentSelect() {
                ...
                if (show) {
                    ...
                    studentSelect.value = ''; // Reset value when hidden
                }
            }
