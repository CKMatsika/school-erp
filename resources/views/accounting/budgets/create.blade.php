            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-medium text-gray-900">{{ __('Create New Budget') }}</h3>
                    ...
                    <form action="{{ route('accounting.budgets.store') }}" method="POST" id="budget-form">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('start_date', date('Y-m-d')) }}" required>
                                @error('start_date')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="end_date" id="end_date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('end_date', date('Y-m-d', strtotime('+1 year'))) }}" required>
                                @error('end_date')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="total_amount" id="total_amount" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" placeholder="0.00" step="0.01" min="0" value="{{ old('total_amount') }}" required>
                                </div>
                                @error('total_amount')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            addButton.addEventListener('click', function() {
                ...
                // Re-number the name attributes if needed
                renumberInputs();
            });
            ...
            // Function to renumber inputs if needed
            function renumberInputs() {
                const rows = document.querySelectorAll('.budget-category-row');
                rows.forEach((row, index) => {
                    // If you need to update name attributes with indexes
                    // row.querySelector('.category-name').name = `categories[${index}]`;
                    // row.querySelector('.category-amount').name = `category_amounts[${index}]`;
                });
            }
