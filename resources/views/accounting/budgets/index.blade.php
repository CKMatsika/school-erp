            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-medium text-gray-900">{{ __('Manage Budgets') }}</h3>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Fiscal Year') }}</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($budgets as $budget)
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $budget->name }}</td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($budget->total_amount, 2) }}</td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('accounting.budgets.show', $budget->id) }}" 
                                                    ...
                                                    <a href="{{ route('accounting.budgets.edit', $budget->id) }}" 
                                                    ...
                                                    <form action="{{ route('accounting.budgets.destroy', $budget->id) }}" 
                                                          method="POST" class="inline-block delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-900" 
                                                                title="{{ __('Delete') }}" onclick="return confirm('{{ __('Are you sure you want to delete this budget?') }}')">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3 flex-1 md:flex md:justify-between">
                                    ...
                                    <p class="mt-3 text-sm md:mt-0 md:ml-6">
                                        <a href="{{ route('accounting.budgets.create') }}" class="whitespace-nowrap font-medium text-blue-700 hover:text-blue-600">
                                            {{ __('Create your first budget') }} <span aria-hidden="true">&rarr;</span>
                                        </a>
                                    </p>
                                </div>
                            </div>
