        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('accounting.sms.templates.update', $template) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Template Name</label>
                            <input type="text" name="name" id="name" value="{{ old('name', $template->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('name')
                        ...
                        <div>
                            ...
                            <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                <option value="payment_reminder" {{ old('type', $template->type) == 'payment_reminder' ? 'selected' : '' }}>Payment Reminder</option>
                                <option value="payment_receipt" {{ old('type', $template->type) == 'payment_receipt' ? 'selected' : '' }}>Payment Receipt</option>
                                <option value="attendance_alert" {{ old('type', $template->type) == 'attendance_alert' ? 'selected' : '' }}>Attendance Alert</option>
                                <option value="event_notification" {{ old('type', $template->type) == 'event_notification' ? 'selected' : '' }}>Event Notification</option>
                                <option value="general" {{ old('type', $template->type) == 'general' ? 'selected' : '' }}>General Message</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="6" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('content', $template->content) }}</textarea>
                        ...
                        <div>
                            <div class="flex items-center mt-4">
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $template->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <label for="is_active" class="ml-2 block text-sm text-gray-700">Active</label>
                            </div>
                        </div>
                        
                        <div class="flex justify-between">
                            <form method="POST" action="{{ route('accounting.sms.templates.destroy', $template) }}" onsubmit="return confirm('Are you sure you want to delete this template?');" class="inline">
                                @csrf
                                @method('DELETE')
